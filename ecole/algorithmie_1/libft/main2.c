...
int             uf_test_strcmp(void)
{
	if (strcmp("abc", "abc") != ft_strcmp("abc", "abc")){
		//	ft_putnbr(strcmp("abc", "abc"));
		D_ERROR}
					if (strcmp("cba", "abc") != ft_strcmp("cba", "abc")){
						//		ft_putnbr(ft_strcmp("cba", "abc"));
								  D_ERROR}
					if (strcmp("abc", "cba") != ft_strcmp("abc", "cba")){
						//				ft_putnbr(strcmp("abc", "cba"));
						D_ERROR}
					if (strcmp("", "") != ft_strcmp("", ""))
                D_ERROR
					return (1);
...
int             uf_test_strstr(void)
{
	...
	if (strstr("neeerd", "eer") != ft_strstr("neeerd", "eer"))
                D_ERROR
...
int     uf_test_memset(void)
{
	...
	while (k <= sizeof(ltab))
	{
		...
		while (i < RANDT)
		{
			...
			temp = memset(itab, i, (k < sizeof(itab)) ? k : sizeof(itab));
			temp2 = ft_memset(itab2, i, (k < sizeof(itab)) ? k : sizeof(itab));
			if (memcmp(itab, itab2, sizeof(itab)) != 0 || temp != itab || temp2 != itab2)
                                D_ERROR
									temp = memset(ctab, i, (k < sizeof(ctab)) ? k : sizeof(ctab));
			temp2 =ft_memset(ctab2, i, (k < sizeof(ctab)) ? k : sizeof(ctab));
			if (memcmp(ctab, ctab2, sizeof(ctab)) != 0 || temp != ctab || temp2 != ctab2)
                                D_ERROR
									temp = memset(ltab, i, (k < sizeof(ltab)) ? k : sizeof(ltab));
			temp2 = ft_memset(ltab2, i, (k < sizeof(ltab)) ? k : sizeof(ltab));
